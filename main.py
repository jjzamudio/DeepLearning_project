...
parser.add_argument('--pretrain_epochs', type=int, default=1, help='number of epochs to pre-train for')
...
for epoch in range(opt.niter):
    for i, data in enumerate(dataloader, 0):
       ...
       # train with real
        ...
        noise = Variable(inputv.data.new(inputv.size()).normal_(0, 0.001))

        output = netD(inputv+noise)
...
#        noisev = Variable(noise)
        ae_input = AutoEncoder.input_to_hidden(inputv/255) #trained autoencoder to take data in [0,1]
        fake = netG(ae_input)
        ...
        #Add noise to Discrimnator input
        #ADD NOISE
        output = netD(fake+noise)
...
np.savetxt('%s/fake_images%d_vae%d.csv' % (opt.outf, epoch,opt.pretrain_epochs),mydata,delimiter=",")
