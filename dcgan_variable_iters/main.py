...
#errD = Variable(torch.randn(1))
#errG = Variable(torch.randn(1))
...
np.savetxt('%s/fake_images%d_variable_gan.csv' % (opt.outf, epoch),mydata,delimiter=",")

#Real images
mydata=(real_cpu).cpu().numpy()
mydata=np.array([mydata[i].flatten() for i in range(mydata.shape[0])])
np.savetxt('%s/real_images%d_variable_gan.csv' % (opt.outf, epoch),mydata,delimiter=",")

#save D_G
D_G_array = np.array(D_G_list)
np.savetxt('%s/D_G_z%d_variable_gan.csv' % (opt.outf, epoch),D_G_array,delimiter=",")

G_losses_array = np.array(G_losses)
np.savetxt('%s/G_losses%d_variable_gan.csv' % (opt.outf, epoch),G_losses_array,delimiter=",")
